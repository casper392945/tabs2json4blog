...
let tabs = [];
let html = "";
let searchValue = ""; // Define searchValue outside the search() function
  ...
  fetch("assets/data/tabs2json4blog.json")
    ...
    .then((data) => {
      const filteredData = data.filter((item) => {
        ...
        const description = item["og:description"];
        return description && typeof description === "string" && description.toLowerCase().includes(searchValue.toLowerCase());
        return String(item["og:description"].toLowerCase().includes(searchValue.toLowerCase()));
      });
